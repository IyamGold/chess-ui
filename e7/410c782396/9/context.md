...
interesting now when i authenticate, it doesn't send me back to /authsuccessful intead it redirects to the localhost http://localhost:53053/callback?REDACTED&REDACTED

### Prompt 79

no the problem is how the redirect page looks. can we customize it?

### Prompt 80

if this is something beyond our control then the only thing we need to do is remove /authsuccessful page completely because the only reason i added it was because i thought we'd be customizing the localhost page.

but since we cant, we should worry about the /authsuccessful page. lets simply redirect to localhost without /authsuccessful because right now having those two pages is redundant

### Prompt 81

okay great so now lets remove mcp-http and reinstall so we can test the auth flow
